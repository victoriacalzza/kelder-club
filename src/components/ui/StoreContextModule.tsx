...
import { novedadesDeTienda, disponibilidadContextual, type Producto } from "@/lib/mock-data";
import { track } from "@/lib/analytics";

/**
 * Home's "Novedades en tu tienda" module: a SHORT carousel of new products in the selected store
 * (not the whole catalog). Header names the store; each product shows ONE contextual availability
 * signal (size-aware). Connects DISCOVERY → PRODUCTO → TIENDA → VISITA FÍSICA.
 */
...
export function StoreContextModule() {
  ...
  // Short carousel of NEW products in the store (newest first), by category chip.
  const activo = chips.find((c) => c.key === chip)!;
  const productos = novedadesDeTienda(tienda.id).filter(activo.test).slice(0, 8);

  return (
    <section aria-label="Novedades en tu tienda">
      {/* This section is NOT the whole catalog — just new arrivals in the selected store. */}
      <div className="flex items-baseline justify-between gap-3">
        <h2 className="text-lg font-semibold text-ink-900">Novedades en tu tienda</h2>
        <button
          onClick={() => {
      ...
      <p className="mt-0.5 text-sm text-ink-500">{tienda.nombre}</p>
        ...
        {productos.map((p) => (
          <div key={p.id} className="w-[42%] min-w-[150px] max-w-[190px] shrink-0 snap-start sm:w-[30%] lg:w-[19%]">
            <ProductPeekCard
              ...
              contexto={disponibilidadContextual(p, tienda.id, tallaMx)}
            />
          </div>
        ))}
        {productos.length === 0 && (
